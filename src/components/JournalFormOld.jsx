...
export default function JournalForm() { 
    ...
    useEffect(() => {
        ...
        if (session) {
            ...
            document.cookie = "visited=true; max-age=31536000"; // Expires in 1 year
            setUser({ credits: 1 });
            }
        }
        else {
    ...
    const goToSelectOracles = () => {
        ...
        if (interpretationSectionRef.current) {
            ...
            const journalButton = document.querySelector('.JournalDream');
            journalButton.classList.add('fade-upwards-in');
    ...
    const goBackToJournal = () => {
        if (journalSectionRef.current) {
            ...
            journalSectionRef.current.classList.remove("hidden");
            journalSectionRef.current.classList.add("fade-downwards-in");
        }
        ...
        if (interpretationSectionRef.current) {
            ...
            journalButton.classList.remove('fade-upwards-in');
    ...
    return (
        <div className="text-white main-content relative h-full">
            {savingDream ? (
                ...
                <div>
                    {!subscribed && (
                        <div className="absolute right-0 top-0 hidden md:block p-2 main-content">
                            <p className="text-right">Dream Credits: {user?.credits}</p>
                            {!user && (
                                <a href='/createAccount' className="underline font-bold">Create an account for 5 dream credits</a>
                            )}
                        </div>
                    )}
                    <div>
                        <div className="journal-section absolute w-full" ref={journalSectionRef}>
                            {user?.name ? (
                        ...
                        <div className="flex justify-center text-3xl items-center JournalDream hidden">
                        ...
                        <div id="interpretation-section" className="hidden" ref={interpretationSectionRef}>
                            ...
                            {user?.credits === 0 && !user?.name ? (
                                <div className="absolute inset-0 flex flex-col md:justify-center items-center">
                                    <span className="text-3xl font-semibold text-center mt-5 md:mt-0">Create a free account to continue to interpret your dreams</span>
                                    <button className="rounded-xl bg-blue-600 dream-button m-2 pl-4 pr-4 justify-center item p-10" onClick={() => window.location.href = '/createAccount'}>Create Account</button>
                                </div>
...
const HowItWorksPopup = () => {
    ...
    return (
        <div className="flex justify-center items-center text-3xl pb-5 p-3 text-center">
            <span className="inline-block">Enter Dream Description
                <span className="pl-2">
                    Below                 
                    <Popup 
                        trigger={<FontAwesomeIcon icon={faInfoCircle} className="ml-2"/>} 
             ...
             {/* <span className="inline-flex items-center">Enter Dream Description Below<FontAwesomeIcon icon={faInfoCircle} className="ml-2"/>
